...
unique_ptr<DeclarationAST> Parser::parse_declaration()
{
    ...
    if (match(TokenType::TOKEN_OPEN_BRACKET))
    {
        ...
        try
        {
            array_size = std::stoi(size_tok.value);
            if (array_size <= 0)
                throw error(size_tok, "Array size must be positive.");
        }
        catch (const std::exception &)
        {
            throw error(size_tok, "Invalid array size literal.");
        }
        if (!match(TokenType::TOKEN_CLOSE_BRACKET))
            throw error(peek(), "Expected ']' after array size.");
    }

    if (match(TokenType::TOKEN_OPERATOR_ASSIGN))
        init_expr = parse_expression();

    if (array_size > 0 && init_expr != nullptr)
    {
        throw error(peek(), "Array declarations with initializer not supported in this compiler subset.");
    }
...
unique_ptr<ExprAST> Parser::parse_factor()
{
    ...
    if (match(TokenType::TOKEN_IDENTIFIER))
    {
        ...
        while (peek().type == TokenType::TOKEN_OPEN_BRACKET)
        {
            ...
            if (!match(TokenType::TOKEN_CLOSE_BRACKET))
                throw error(peek(), "Expected ']' after array index.");

            base = make_unique<IndexExprAST>(move(base), move(index_expr));
        }
